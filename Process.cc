...
{
    int num;
    char Input[100], Output[100], RootFile[100], directory[100], temp[100];
    char n[100];
    ofstream out;
    num = GetFileName(argc, argv, Input, Output, RootFile);
...
int Initialize()
{
    ...
    for (int i = 0; i < 112; i ++)
    {
    	...
    	in >> map_PMT[i];
	in >> map_amp[i];
	//cout << map_ch[i] << "  " << map_di[i] << "  " << map_PMT[i] << "  " << map_amp[i]<< "  " << endl;
    }
    return 0;
}
int GetWaveform()
...
int GetIntegralMap()
{
	...
	if (temp_w[ChId - 256]->GetBinContent(p - i) < 0.1 * temp_w[ChId - 256]->GetBinContent(p))
	{
	    ...
	    //cout << Peak_Start[ChId - 256] << endl;
	    //cout << temp_w[ChId - 256]->GetBinContent(p - i - 2) << endl;
	    break;
	}
    }
    for (int i = 0; i < 150; i ++)
...
int FillCharge()
{
    ...
    //if (charge[ChId - 256] < 0)
    //    charge[ChId - 256] = -5;
    charge_spectrum[ChId - 256]->Fill(charge[ChId - 256]);
    //PENum[ChId - 256]->Fill(charge[ChId - 256] / Gain[256]);
    Ch_count ++;
    return 0;
}
int GetGain()
...
int GetFileName(int argc, char **argv, char *input_Filename, char *output_FileName, char *rootfile)
{
    char temp[200];
    int num;
    if (argc == 6)
    ...
    char directory[200];
    strcpy(directory, INPUT_DIRECTORY);
    sprintf(temp, "%s/%s", directory, input_Filename);
    strcpy(input_Filename, temp);*/
    //strcpy(directory, OUTPUT_DIRECTORY);
    //sprintf(temp, "%s/%s", directory, output_FileName);
    return num;
}
